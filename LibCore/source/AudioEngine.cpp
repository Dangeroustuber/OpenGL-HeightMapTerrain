...
void AudioEngine::createListener() {
...
int AudioEngine::initializeStruct() {
		
		pSampleData = drwav_open_file_and_read_pcm_frames_s16("sounds/Exam-song-Woods.wav", &stereoData.channels, &stereoData.sampleRate, &stereoData.totalPCMFrameCount, nullptr);
		if (pSampleData == NULL) {
...
void AudioEngine::generateStereoBuffer() {
...
void AudioEngine::initializeSourceState() {
	alec(alSourcePlay(stereoSource));
	alec(alGetSourcei(stereoSource, AL_SOURCE_STATE, &sourceState));
}

void AudioEngine::runSound() {
	alec(alGetSourcei(stereoSource, AL_SOURCE_STATE, &sourceState));
}

void AudioEngine::freeResources() {
...
AudioEngine::~AudioEngine() {freeResources();}
