...
smooth in float heightCol; // this could also be solved with a sampler 2d texture but this is way easier.
in vec4 vCol;
in vec2 TexCoord;
in vec3 Normal;
in vec3 FragPos;

//uniform sampler2D heightSamples[2];
...
vec3 colors[3];	// Create a vector to hold the colours for seperation
...
// function that returns a vector that will color framgments based on height.
vec3 colorHeight() {	
	colors[0] = vec3(0.8,0.4,0.2);			// define what color i want
	colors[1] = vec3(0.1,0.5,0.3);
	colors[2] = vec3(0.8,0.9,1.0);

	if(heightCol < 30.0) {
		return colors[0];
	} 
	if(heightCol < 50.0 && heightCol > 30.0) {
		return colors[1];
	}  
	if (heightCol > 50.0 && heightCol < 115.0) {
		return colors[2];
	}
...
void main()
{
	...
	vec3 colorT = colorHeight();

	vec4 actualColor = vec4(colorT, 1.0f); // perserve RGBA, important

	vec4 actualactualColor = CalcDirectionalLight();

	vec4 finalColour = CalcDirectionalLight();
	finalColour += CalcPointLights();
	finalColour += CalcSpotLights();
	
	colour = texture(theTexture, TexCoord) * actualColor * finalColour;
}